---
title: R Notebook for *A corpus-based study of constructional equivalence for the
  Indonesian translation of ROB and STEAL based on the __OpenSubtitle__ English-Indonesian Parallel Corpus*
...
### Analysis for the lexical equivalence for ROB and STEAL

```{r lexical-equivalence-database, include = FALSE, message = FALSE}
rob_equivalence_df <- read_tsv("https://raw.githubusercontent.com/gederajeg/rob-steal-parallel-corpora/main/SENASTEK/SENASTEK_data_rob1.txt")
steal_equivalence_df <- read_tsv("https://raw.githubusercontent.com/gederajeg/rob-steal-parallel-corpora/main/SENASTEK/SENASTEK_data_steal1.txt")
```


This study is based on data in Rajeg et al. [-@rajeg_pemanfaatan_2021]. [Table \@ref(tab:lex-equivalence-rob)](#lex-equivalence-rob) shows the lexical equivalence in Indonesian for word forms of the lemma ROB and [Table \@ref(tab:lex-equivalence-steal)](#lex-equivalence-steal) shows the same data for the lemma STEAL.

```{r lexical-equivalence-table}
rob_lex_equiv <- rename(summarise(arrange(group_by(count(rob_equivalence_df, NODE, TRANSLATION_NODE), NODE), desc(n), desc(TRANSLATION_NODE)), `Indonesian translation` = paste(paste("*", TRANSLATION_NODE, "* (", n, ")", sep = ""), collapse = "; ")), `English form` = NODE)

steal_lex_equiv <- rename(summarise(arrange(group_by(count(steal_equivalence_df, NODE, TRANSLATION_NODE), NODE), desc(n), desc(TRANSLATION_NODE)), `Indonesian translation` = paste(paste("*", TRANSLATION_NODE, "* (", n, ")", sep = ""), collapse = "; ")), `English form` = NODE)
```

```{r lex-equivalence-rob}
knitr::kable(rob_lex_equiv, caption = "Indonesian lexical equivalence for word forms of the lemma ROB")
```


```{r lex-equivalence-steal}
knitr::kable(steal_lex_equiv, caption = "Indonesian lexical equivalence for word forms of the lemma STEAL")
```
